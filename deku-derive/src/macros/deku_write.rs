...
fn emit_struct(input: &DekuData) -> Result<TokenStream, syn::Error> {
    ...
    if has_size_hint {
        if let Some(magic) = &input.magic {
            size_hint.extend(quote! { + #magic.len() });
        }
        ...
        if is_sized {
            tokens.extend(quote! {
                ...
                impl #imp ::#crate_::DekuWriteSized for #ident #wher {
                    const BIT_SIZE: usize = #size_hint;
                }
            });
        } else {
            tokens.extend(quote! {
...
fn emit_enum(input: &DekuData) -> Result<TokenStream, syn::Error> {
    ...
    let mut size_hint = if let Some(id_type) = id_type.as_ref() {
    ...
    let mut size_hint_matcher = quote! {};
    ...
    for variant in variants {
        ...
        if has_size_hint {
            ...
            for (i, field) in variant.fields.iter().enumerate() {
                emit_field_size_hint(&crate_, i, field, None, &mut size_hint_variant, is_sized)?;
            }
            if is_sized {
                size_hint_matcher.extend(quote! { #size_hint_variant, });
            } else {
    ...
    if is_sized {
        tokens.extend(quote! {
            ...
            impl #imp ::#crate_::DekuWriteSized for #ident #wher {
                const BIT_SIZE: usize = #size_hint + {
                    let all_vars = [#size_hint_matcher];
...
fn emit_field_size_hint(
    ...
    if let Some(bits) = &field.pad_bits_before {
        size_hint.extend(quote! { + #bits });
    } else if let Some(bytes) = &field.pad_bytes_before {
        size_hint.extend(quote! { + #bytes * 8 });
    }
    ...
    if let Some(bits) = &field.bits {
        ...
        size_hint.extend(quote! { + <#ty as #crate_::DekuWriteSized>::BIT_SIZE });
    } else {
