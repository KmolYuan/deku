...
macro_rules! ImplDekuPointerSized {
    ($wrapper:ident<$($a:lifetime,)? $T:ident $(: $bound:tt)?>) => {
        impl<$($a,)? $T: crate::DekuReadSized $(+ $bound)?> crate::DekuReadSized for $wrapper<$($a,)? $T> {
            const BIT_SIZE: usize = <T as crate::DekuReadSized>::BIT_SIZE;
        }
        impl<$($a,)? T: crate::DekuReadSizeHint $(+ $bound)?> crate::DekuReadSizeHint for $wrapper<$($a,)? $T> {
        ...
        impl<$($a,)? T: crate::DekuWriteSized $(+ $bound)?> crate::DekuWriteSized for $wrapper<$($a,)? $T> {
            const BIT_SIZE: usize = <T as crate::DekuWriteSized>::BIT_SIZE;
        }
        impl<$($a,)? T: crate::DekuWriteSizeHint $(+ $bound)?> crate::DekuWriteSizeHint for $wrapper<$($a,)? $T> {
